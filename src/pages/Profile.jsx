        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-start gap-6">
          ...
          <div className="relative">
            <img
              ...
              className="w-24 h-24 rounded-2xl border-2 border-primary/50 object-cover shadow-[0_0_20px_rgba(221,183,255,0.3)]"
            />
            <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-green-400 border-2 border-surface flex items-center justify-center">
          ...
          <div className="flex-1 text-center sm:text-left">
            <h1 className="text-headline-md font-headline-md text-on-surface">{user.displayName}</h1>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded border border-tertiary/30 text-tertiary text-sm hover:bg-tertiary/10 transition-colors"
            >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <StatsSection title="Anime" icon="movie" color="text-primary" stats={animeStats} labels={{
              watching: 'Viendo', completed: 'Completado', planned: 'Planeado',
              on_hold: 'En pausa', dropped: 'Abandonado',
            }} />
            <StatsSection title="Manga" icon="auto_stories" color="text-secondary" stats={mangaStats} labels={{
          ...
          <div className="flex flex-col gap-4">
            <div className="glass-card rounded-xl border border-outline-variant/20 p-4">
              <h3 className="text-headline-sm font-headline-sm text-on-surface mb-4">Acceso rápido</h3>
              <div className="flex flex-col gap-2">
                {[
                  { to: '/my-list?media=anime', icon: 'movie', label: 'Mi lista anime' },
            ...
            {reviews.length > 0 && (
              <div className="glass-card rounded-xl border border-outline-variant/20 p-4">
                <h3 className="text-headline-sm font-headline-sm text-on-surface mb-4">Mis reseñas</h3>
                <div className="flex flex-col gap-3">
                  {reviews.slice(0, 3).map(r => (
                    <Link key={r.id} to={`/${r.entryType}/${r.entryId}`}
                      ...
                      <p className="text-sm font-medium text-on-surface line-clamp-1">{r.entryTitle}</p>
                      ...
                      <p className="text-xs text-on-surface-variant line-clamp-2">{r.reviewText}</p>
                    </Link>
                  ))}
                </div>
              </div>
  ...
  return (
    <div className="glass-card rounded-xl border border-outline-variant/20 p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-on-surface-variant text-sm">({stats.total} total)</span>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
        {entries.map(([key, label]) => (
          <div key={key} className="flex flex-col items-center p-3 rounded-lg bg-surface-container-high/40 border border-outline-variant/20">
            <span className={`text-xl font-black ${color}`}>{stats[key] || 0}</span>
            <span className="text-label-md font-label-md text-on-surface-variant text-center mt-1">{label}</span>
          </div>
        ))}
      </div>
    </div>
